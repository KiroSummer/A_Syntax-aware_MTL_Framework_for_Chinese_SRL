...
if __name__ == '__main__':
  ...
  _, sys_predicates, predicted = read_conll_prediction(sys.argv[1])
  ...
  sentence_id = -1
  for s0, props, g0, sys_props, p0 in izip(sentences, predicates, gold, sys_predicates, predicted):
    num_gold, num_predicted, num_matched = 0, 0, 0

    # print "s0", s0, '\n', "props", props, '\n', "g0", g0, '\n', 's0', s0
    # compute num_gold
    for prop_id, gold_args in izip(props, g0):
      gold_args = [s for s in gold_args if s[0] != 'V' and not 'C-V' in s[0]]
      num_gold += len(gold_args)
    # compute num_sys
    for sys_prop_id, sys_args in izip(sys_props, p0):
      sys_args = [s for s in sys_args if s[0] != 'V' and not 'C-V' in s[0]]
      num_predicted += len(sys_args)
    # compute matched
    for prop_id, gold_args in izip(props, g0):
      gold_args = [s for s in gold_args if s[0] != 'V' and not 'C-V' in s[0]]
      for sys_prop_id, sys_args in izip(sys_props, p0):
        if prop_id == sys_prop_id:
          sys_args = [s for s in sys_args if s[0] != 'V' and not 'C-V' in s[0]]
          pred_matched = [find(p, gold_args) for p in sys_args]
    ...
    if num_gold == 0:
      if num_predicted != 0:
        ...
        print '\t', sentence_id, '\t', num_gold, '\t', 0, '\t', "%.2f"%precision, '\t', "%.2f"%0.0, '\t', num_matched, '\t', num_matched, '\t', num_gold, '\t', "0 0 0 0"
      continue
    if num_predicted == 0:
