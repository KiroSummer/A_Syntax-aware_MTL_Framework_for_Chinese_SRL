...
class DocumentState(object):
    ...
    def finalize_sentence(self):
        ...
        for pred, args in itertools.izip(self.predicate_buffer, self.argument_buffers):
            ...
            if len(args) == 0:  # include those predicates which has no arguments. (CoNLL-2009)
                self.srl[-1].append((pred, pred, pred, "V"))
        self.predicate_buffer = []
        self.argument_buffers = []
        self.argument_stacks = []
...
def handle_bit(word_index, bit, stack, spans, label_set):
    ...
    spans.append((word_index, word_index, label))
...
def handle_line(line, document_state, language, num_cols, labels, stats):
    ...
    pos = row[4]  # gold or auto? never mind@kiro
    parse = row[10]  # syntax label
    lemma = row[2]
    args = row[14:]

    predicate_sense = row[13]  # there is no predicate sense in CPB1.0 @kiro
...
if __name__ == "__main__":
    ...
    # python dependency_srl_to_json.py CoNLL2009-ST-Chinese-train.txt train.chinese.conll09.jsonlines 13
